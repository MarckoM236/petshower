...
public class Sentencias_sql {
    ...
    public boolean Update(String datos[], String upsql){
      boolean estado = false;
       try {
            ps = con.conectado().prepareStatement(upsql);
            for(int i=0; i<=datos.length-1;i++){
                ps.setString(i+1, datos[i]);
            }
            ps.execute();
    ...
    public boolean Delete(String delsql){
    boolean estado = false;
    
        try {
            
            ps = con.conectado().prepareStatement(delsql);
    ...
    return data;
     ...
     //Consulta factura:
     public Object [][] GetFactura(String sql,String colName[], String cod_factura){
     
  int registros = 0;
      
      try{
         ps = con.conectado().prepareStatement("select count(*) as total from detalleFactura where cod_factura='"+cod_factura+"';");
         res = ps.executeQuery();
         res.next();
         registros = res.getInt("total");
         res.close();
  ...
  public Object[] poblar_combox_Vendedor(String tabla, String campo, String sql){
       int registros = 0;      
      try{
         ps = con.conectado().prepareStatement("SELECT count(*) as total FROM " + tabla +" where Cod_rol=2" );
    ...
    return data;
     ...
     //para el login
      ...
      public Object [] login(String tabla, String campo, String sql){
